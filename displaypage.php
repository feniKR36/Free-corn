    ...
    <form method="POST" action="process.php">
        ...
        <script>
            function rageQuit(event) {
                ...
                // 2. Check if all the required fields are filled out
                if (form.checkValidity()) {
                    // 3. Stop the form from sending data to process.php
                    event.preventDefault();

                    // 4. Try to close the browser tab
                    window.close();

                    // 5. The Fallback: If the browser blocks window.close(), we wipe the screen!
                    document.body.innerHTML = "<h1 style='color:white; margin-top:20%; font-family:Arial;'>TAB 'CLOSED'. NO CORN FOR YOU.</h1>";
                    document.body.style.backgroundColor = "black";
                }
                // If the form is NOT valid, the script does nothing. 
                // The browser takes over and shows the standard "Please fill out this field" popup!
            }
        </script>
    </form>
